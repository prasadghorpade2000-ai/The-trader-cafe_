...
if st.sidebar.button("💾 Force Save Data"):
    save_data()
    st.sidebar.success("All data saved to file!")
...
# --- 5. SALES REPORT ---
    ...
    if st.session_state.sales or st.session_state.daily_archive:
        try:
            ...
            with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                ...
                inv_list = [{"Item": k, **v, "Remained": v['added']-v['sold']} for k, v in st.session_state.inventory.items()]
                pd.DataFrame(inv_list).to_excel(writer, sheet_name='Inventory', index=False)
            ...
            st.warning("⚠️ Excel Download is locked. Add 'xlsxwriter' to requirements.txt on GitHub.")
        st.divider()
    ...
    with tab1:
        ...
        else: st.info("No sales yet today.")
