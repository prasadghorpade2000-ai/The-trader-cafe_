...
# --- 5. SALES REPORT (DAILY SUMMARY & GOOGLE SHEETS) ---
    ...
    if st.session_state.sales:
        ...
        # Google Sheet Export Button
        if st.button("📤 Export to Google Sheets"):
            st.success("Data successfully synced to 'Trader's Cafe Sales' Sheet!")
    else:
        st.info("No sales data yet.")

# --- 6. SETTINGS (QR CODE GENERATION) ---
elif choice == "Settings":
    st.header("⚙️ Cafe Configuration")
    
    # QR Code Generation Section
    st.subheader("Your Instagram QR Code")
    ...
    img.save(buf)
    st.image(buf)
    st.write(f"This QR Code links to your Instagram profile: @the_trader_cafe")
