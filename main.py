...
if choice == "Dashboard":
    ...
    with c2:
        ...
        if current_order:
            ...
            # --- INDIVIDUAL ITEM CANCEL WITH REASON ---
            for idx, entry in enumerate(current_order):
                col_item, col_btn = st.columns(2)
                col_item.write(f"• {entry['Item']} (₹{entry['Price']})")
                
                # Popover for Reason (Clean UI)
            ...
            with st.expander("🚫 Cancel Full Order"):
                ...
                if st.button("⚠️ Confirm Full Cancel"):
                    ...
                    st.session_state.tables[selected_tab] = []
                    st.warning("Full order cancelled.")
                    st.rerun()
    ...
    if st.session_state.sales:
        ...
        with t2:
            ...
            if not cancel_df.empty:
                st.write("### Why are orders being cancelled?")
